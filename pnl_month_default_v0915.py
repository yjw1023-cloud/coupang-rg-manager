...
def patch_source(source: str) -> str:
    ...
    # Add a separate menu entry for the existing source-file view.
    menu = '        "📈  잠정손익",\n'
    if menu in source and '        "📄  자료별 잠정손익",\n' not in source:
        source = source.replace(menu, menu + '        "📄  자료별 잠정손익",\n', 1)

    # Rename the existing branch in place, then insert a tiny monthly branch
    # immediately before it. No legacy body is copied or re-indented.
