...
def render_grouped_sidebar(st_obj, options, default_page=None):
    ...
    if isinstance(groups, list):
        for title, items in groups:
            if sales_label in items and str(title) != sales_title:
                items[:] = [x for x in items if x != sales_label]
        if not any(str(title) == sales_title for title, _items in groups):
            insert_at = 1 if groups else 0
            groups.insert(insert_at, (sales_title, [sales_label]))
        else:
            for title, items in groups:
