...
# v0.5 transitional loader: preserve the known-good v0.4 application source,
# apply the v0.5 dashboard patch in memory, then run it normally.
# This lets the built-in v0.4 updater move to v0.5 without touching user data.
...
if not BASE_APP.exists():
    ...
    if BACKUP_APP.exists():
        ...
        raise RuntimeError(
            "v0.5 기본 코드(app_v0.4.py)를 찾을 수 없습니다. "
            "v0.4 프로그램의 업데이트 메뉴에서 v0.5를 적용해 주세요."
        )
...
if old_monthly not in source:
    raise RuntimeError("v0.5 대시보드 패치를 적용할 기준 코드를 찾지 못했습니다.")

source = source.replace(old_monthly, new_monthly, 1)
source = source.replace('st.sidebar.caption("v0.4 · auto updater")', 'st.sidebar.caption("v0.5 · monthly dashboard")', 1)
