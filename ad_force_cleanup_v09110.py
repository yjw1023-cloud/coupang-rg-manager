"""RG Manager v0.9.110 one-time forced cleanup for one explicitly authorized ad report.

The user explicitly instructed removal of the exact report:
A00577001_pa_total_campaign_20260801_20260811.xlsx (2026-08-01~2026-08-11)

This migration removes that exact record from both canonical provisional-ad tables
and generic imports/ad_performance, regardless of historical source linkage.
It then writes a DB migration flag so a later intentional re-upload of the same
period/file is not deleted again on future startups.
"""
...
FLAG = "v0.9.110_force_delete_20260801_20260811_ad"
...
def apply(core, db=None):
    ...
    with core._conn(db) as c:
        ...
        # Canonical provisional advertising report.
        if _exists(c, "provisional_ad_report_imports"):
            ...
            for r in rows:
                ...
                c.execute(
                    "DELETE FROM provisional_ad_report_imports WHERE id=?",
                    (rid,),
                )
                result["canonical_deleted"] += 1

        # Generic '새 자료 반영' history and its option-level ad rows.
        if _exists(c, "imports"):
            ...
            for r in legacy:
                ...
                if _exists(c, "ad_performance"):
                    cur = c.execute(
                        "DELETE FROM ad_performance WHERE import_id=?", (iid,)
                    )
                    try:
                        result["ad_rows_deleted"] += int(cur.rowcount or 0)
                    except Exception:
                        pass
                c.execute(
